...
function useAuthCodeGrant (done) {
  ...
  this.model.getAuthCode(code, function (err, authCode) {
    ...
    if (authCode.codeChallenge) {
      return self.model.verifyCodeChallenge(authCode.codeChallenge, authCode.codeChallengeMethod, self.req, function(pkceErr, matches) {
        if (pkceErr) {
          return done(error('invalid_request', 'Code challenge method not supported'));
        }
        if (!matches) {
          return done(error('invalid_grant', 'Code challenge failed'));
        }
        done();
      });
    }
